...
static Node* partition(LinkedList* this, Node* headNode, Node* endNode, Node** newHead, Node** newEnd, int (*pFunc)(void*, void*), int order);
...
static Node* quicksort(LinkedList* this, Node* headNode, Node* endNode, int (*pFunc)(void*, void*), int order)
{
    ...
    Node* pivot = partition(this, headNode, endNode, &newHead, &newEnd, pFunc, order);
    ...
    if(newHead != pivot)
    {
        ...
        newHead = quicksort(this, headNode, endNode, pFunc, order);

        tmpNode = getTail(newHead);
...
static Node* partition(LinkedList* this, Node* headNode, Node* endNode, Node** newHead, Node** newEnd, int (*pFunc)(void*, void*), int order)
{
    ...
    {
        swap = pFunc(auxNodeCurr->pElement, pivot->pElement);
        if (swap < 0)
        {
            swap = 0;
        }
